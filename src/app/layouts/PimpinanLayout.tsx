...
export function PimpinanLayout() {
      ...
      >
        ...
        {showLabels && (
          <div className="mx-3 mb-3 px-3 py-2 rounded-xl" style={{ background: "#EFF6FF" }}>
            ...
            <p className="text-sm font-medium text-blue-700 capitalize">Pimpinan</p>
          </div>
        )}

        <nav className="flex-1 px-2 space-y-0.5 overflow-y-auto">
          {pimpinanNav.map(({ path, label, icon: Icon }) => (
            <NavLink
              key={path}
              to={path}
        ...
        <div className="p-3 border-t border-slate-100 relative">
          ...
          {showProfile && (
            ...
            >
              <button
                onClick={() => { navigate("/pimpinan/profil"); setShowProfile(false); setMobileOpen(false); }}
